...
#ps import sample_farthest_points
from natsort import natsorted
def sample_farthest_points(points, K):
    """
    用纯 PyTorch 完美平替 PyTorch3D 的最远点采样 (FPS)
    points: (B, N, 3) 或者是 (N, 3)，如果是 (N, 3) 会被自动包成 (1, N, 3)
    K: 采样的点数
    返回: (sampled_points, sampled_indices)
    """
    ...
    sampled_points = torch.gather(points, 1, centroids.unsqueeze(-1).expand(-1, -1, C))
