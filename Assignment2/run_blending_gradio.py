...
def create_mask_from_points(points, img_h, img_w):
    ...
    # 4. 最后转回 numpy 数组返回，供后续 torch 计算使用
...
def cal_laplacian_loss(foreground_img, foreground_mask, blended_img, background_mask):
    ...
    fg_lap = torch.nn.functional.conv2d(foreground_img, laplacian_kernel, padding=1, groups=3)
    blended_lap =torch.nn.functional.conv2d(blended_img, laplacian_kernel, padding=1, groups=3)

    fg_mask_bool = foreground_mask.bool().expand(-1, 3, -1, -1)
    background_mask_bool = background_mask.bool().expand(-1, 3, -1, -1) 

    loss = torch.mean((fg_lap[fg_mask_bool] - blended_lap[background_mask_bool]) ** 2)  
